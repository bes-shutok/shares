...
#        if indices: for i in sorted(indices, reverse=True): del tokens_list[i]
            #tokens_list[i] = tokens_list[i].split(" ")[0]
    return target_list
...
def parse_data(path: Union[str, Path[str]]) -> List[InciRecord]:
    print("This fine will be parsed.")
    print(path)
    ...
    with open(path, 'r') as read_obj:
        ...
        for row in csv_dict_reader:
            if row[name_col] != "":
                ...
                incis.append(InciRecord(row[name_col], cas, ecs, refs))
